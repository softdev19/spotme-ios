...
class MyDealsBrowseCell: UITableViewCell {

    @IBOutlet weak var profileImage: CircleImageView!
    
    @IBOutlet weak var request_msg: UILabel!
    ...
    var offer_model=RequestsModel()
    var currentUser=UserModel()
    var selectedUser=UserModel()
    @IBOutlet weak var messageBtn: CardView!
    
    func setOffer(offer:RequestsModel,this:UIViewController,position:Int)
    {
        ...
        request_msg.text=offer.request_message
    ...
    @IBAction func profilePic_Pressed(_ sender: Any) {
        self.this.performSegue(withIdentifier: "BrowseToProfile", sender: selectedUser)
    }
    @IBAction func accept_pressed(_ sender: Any) {
        ...
        if(stringOne)
        {
            ...
            popupViewController.universal=true
            popupViewController.universal_object=self.offer_model
    ...
    @IBAction func reportSpamPressed(_ sender: Any) {
        if let popupViewController = UIStoryboard(name: "Home", bundle: nil).instantiateViewController(withIdentifier: "DialogReportSpamViewController") as? DialogReportSpamViewController {
        popupViewController.modalPresentationStyle = .custom
        popupViewController.modalTransitionStyle = .crossDissolve
    ...
    @IBAction func onCounterPressed(_ sender: Any) {
        let defaults = UserDefaults.standard
        let stringOne:Bool = defaults.bool(forKey: "charges_enabled")
        if(stringOne)
        {
            self.this.performSegue(withIdentifier: "MainToCounter2", sender: offer_model)
        }else{
            let alertController = UIAlertController(title: "Alert", message: "Please complete your payment setup before sending request", preferredStyle: .alert)
                let OKAction = UIAlertAction(title: "OK", style: .default, handler: {(action) in
                    self.this.navigationController?.popViewController(animated: true)
